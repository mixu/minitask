...
CacheFile.prototype.path = function(key, value) {
  ...
  return this._set([ 'path', key ], path.normalize(value));
};
...
CacheFile.prototype._getActual = function() {
  if(this.opts.method == 'stat') {
    var stat = fs.statSync(this.opts.path),
        obj = { size: stat.size, mtime: parseInt(stat.mtime.getTime(), 10) };
    return obj;
  } else {
...
CacheFile.prototype._rehash = function() {
  ...
  // cannot use this._get as it calls validation which fails
  function get(key) {
    return self.opts.cache._get([ self._prefix ].concat( key ))
  }
